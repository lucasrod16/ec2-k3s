...
ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no -o IdentitiesOnly=yes ubuntu@"${public_ip}" 'kubectl config view --minify=true --raw=true > ./kubeconfig.yaml'
scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no -o IdentitiesOnly=yes ubuntu@"${public_ip}":./kubeconfig.yaml .
